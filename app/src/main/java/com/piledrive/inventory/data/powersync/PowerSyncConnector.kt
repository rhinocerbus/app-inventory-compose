...
class PowerSyncSupabaseConnector @Inject constructor(
		...
		supabaseClient.httpClient.httpClient.plugin(HttpSend).intercept { request ->
			...
			if (response.status.value == 400) {
				...
				try {
					...
					Timber.e("Failed to parse error response: $e")
				}
			}
			resp
		}
	...
	fun session(): UserSession? = supabaseClient.auth.currentSessionOrNull()
		...
		return runWrappedSuspending {
			...
			try {
				...
				if (errorCode != null && PostgresFatalCodes.isFatalError(errorCode.toString())) {
					 ...
					 */
					Timber.e("Data upload error: ${e.message}")
					Timber.e("Discarding entry: $lastEntry")
					transaction.complete(null)
					return@runWrappedSuspending
				}

				Timber.e("Data upload error - retrying last entry: $lastEntry, $e")
				throw e
			}
		}
	}
